...
def get_data(client_socket):
    ...
    # error_data = int.from_bytes(from_client_msg, 'big', signed=True)
    error_data = int(from_client_msg.decode("gbk"))
    return error_data
        ...
        while client is not None:
            if (datetime.now() > last_update + timedelta(milliseconds = update_interval * 1000)):
                ...
                if current_alt > final_alt:
                    ...
                    if error_data > 0 and error_data <3 :
                ...
                send_data = client_socket.send((" 服务端：客户端你好，服务器端收到！").encode("gbk"))
